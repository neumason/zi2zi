...
def draw_example(ch, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes):
    ...
    srcexample_img = Image.new("RGB", (canvas_size, canvas_size), (255, 255, 255))
    srcexample_img.paste(dst_img, (0, 0))

    dstexample_img = Image.new("RGB", (canvas_size, canvas_size), (255, 255, 255))
    dstexample_img.paste(src_img, (0, 0))

    return srcexample_img,dstexample_img
...
def font2img(src, dst, charset, char_size, canvas_size,
    ...
    for c in charset:
        if count == sample_count:
            break
        e,r= draw_example(c, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes)
        if e:
            e.save(os.path.join(sample_dir, "A%d_%04d.jpg" % (label, count)))
            r.save(os.path.join(sample_dir, "B%d_%04d.jpg" % (label, count)))
            count += 1
            if count % 100 == 0:
